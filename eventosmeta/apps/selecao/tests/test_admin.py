...
class BaseAdminActionTest:
    """Classe base para configurar o ambiente de teste para actions do admin."""
...
class TestMatricularAlunosActionCapacity(BaseAdminActionTest):
    ...
    def test_matricular_alunos_capacidade_ultrapassada(self, usuario_staff):
        """
        Deve exibir erro e não matricular se a quantidade de selecionados
        ultrapassa a capacidade da turma.
        """
    ...
    def test_matricular_alunos_capacidade_exata(self, usuario_staff):
        """
        Deve matricular com sucesso se a quantidade de selecionados
        é igual à capacidade da turma.
        """
        evento = EventoFactory()
...
@pytest.mark.django_db
class TestMatricularAlunosActionValidation(BaseAdminActionTest):
    ...
    def test_validacao_turma_nao_pertence_ao_evento(self, usuario_staff):
        ...
        TurmaFactory(evento=evento1, capacidade=2)  # <-- turma do evento1, para passar a validação inicial
        turma_evento2 = TurmaFactory(evento=evento2, capacidade=1)  # turma do evento2, que será passada errada
...
@pytest.mark.django_db
class TestMatricularAlunosActionSuccess(BaseAdminActionTest):
...
@pytest.mark.django_db
class TestMatricularAlunosActionErrorHandling(BaseAdminActionTest):
    ...
    def test_transacao_atomica_rollback_on_matricula_save_error(self, usuario_staff):
        evento = EventoFactory()
        turma = TurmaFactory(evento=evento, capacidade=3)  # capacidade folgada
