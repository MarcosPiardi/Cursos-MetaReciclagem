...
Descrição: Services do app ACADÊMICO - Lógica de matrículas, avaliações e gestão
Data: 12/01/2026
Atualização: Unificado métodos existentes + novos métodos para gestão de matrícula
"""
...
class MatriculaService:
    ...
    # =========================================
    # NOVOS MÉTODOS (GESTÃO DE MATRÍCULA)
    # Adicionados em 12/01/2026
    # =========================================
    
    @staticmethod
    @transaction.atomic
    def matricular_por_inscricoes(inscricoes_ids, usuario=None):
        """
        Realiza matrícula de múltiplos alunos a partir de IDs de inscrições
        Usado pela interface de Gestão de Matrícula
        ...
        for inscricao in inscricoes:
            try:
                # Verificar se já existe matrícula ativa para este evento
                matricula_existente = Matricula.objects.filter(
                ...
                if matricula_existente:
                    ...
                    logger.info(
                        f"Aluno {inscricao.interessado.nome} já possui matrícula "
                        f"no evento {inscricao.evento.nome}"
                    )
                    continue
                
                # Buscar turma padrão do evento (primeira turma disponível)
                turma = Turma.objects.filter(evento=inscricao.evento).first()
                
                if not turma:
                    erros.append(
                        f"Evento {inscricao.evento.nome} não possui turmas cadastradas"
                    )
                    continue
                ...
                if not disponivel:
                    erros.append(
                        f"Turma {turma.nome} do evento {inscricao.evento.nome} não possui vagas disponíveis"
                    )
                    continue
                ...
                matricula = Matricula.objects.create(
                    ...
                    observacoes=f"Matriculado via Gestão de Matrícula por {usuario.username if usuario else 'sistema'}"
                )
                ...
                logger.info(
                    ...
                    f"Turma: {turma.nome} | "
                    f"Usuário: {usuario.username if usuario else 'sistema'}"
                )
                
            except Exception as e:
                erros.append(
                    f"Erro ao matricular {inscricao.interessado.nome}: {str(e)}"
                )
                logger.error(
                    f"Erro ao matricular aluno {inscricao.interessado.nome}: {e}"
                )
    ...
    def alterar_status_inscricao(inscricoes_ids, novo_status_nome, usuario=None):
        ...
        Usado para marcar inscrições como Cancelado, Expirado, Desistente, Não Localizado
        ...
        try:
            ...
            raise ValueError(
                f"Status '{novo_status_nome}' não encontrado em StatusInscricao"
            )
        ...
        for inscricao in inscricoes:
            try:
                status_anterior = inscricao.status.nome if inscricao.status else 'Sem status'
                ...
                erros.append(
                    f"Erro ao alterar status de {inscricao.interessado.nome}: {str(e)}"
                )
